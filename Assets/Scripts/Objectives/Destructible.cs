...
public class Destructible : MonoBehaviour
{
    ...
    [SerializeField]
    private float PieceTimeSinking = 5f;
    [SerializeField]
    ...
    public void Explode()
    {
        ...
        GameObject brokenInstance = Instantiate(BrokenPrefab, transform.position, BrokenPrefab.transform.rotation);
    ...
    private IEnumerator FadeOutRigidBodies(Rigidbody[] Rigidbodies)
    {
        ...
        while(time < PieceTimeSinking)
        {
            float step = Time.deltaTime * PieceFadeSpeed; 
            foreach (Renderer renderer in renderers)
            {
                renderer.transform.Translate(Vector3.down * (step), Space.World);
            }
